...
const Kid = (props) => {
  ...
  const { tagData2, getTagData2, settingData, getSettingData, destinationData, getDestinationData, header, tagtitle} = props;
  const [loader, setLoader] = useState(true);
  const refferencr = useRef(null);
    console.log('kids=', tagData2);

  const mainTagData = [];
 tagData2 && tagData2.map((data, index) => {
...
const mapStateToProps = ({ user: { tagData2, settingData, destinationData } }) => ({
  tagData2,
  settingData,
  destinationData,
});
...
const mapDispatchToProps = {
  getTagData2: userAction.getTagData2,
  getSettingData: userAction.getSettingData,
  getDestinationData: userAction.getDestinationData,
};
export default connect(mapStateToProps, mapDispatchToProps)(withRouter(Kid));
